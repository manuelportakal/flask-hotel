# gerekli kutuphaneleri cekiyoruz
import datetime
import hashlib
...
# kullanilacak global degiskenleri tanimliyoruz
sorgu_kullanici_id = None
sorgu_kazanilan_puan = 0
sorgu_kullanici_puan = 0


# login erisim duzeyi icin fonksiyon tanimliyoruz
# kosulun saglanmamasi durumunda uyari verip login
# fonksiyonuna yonlendiriyoruz
def login_required(f):
    ...
    def wrap(*args, **kwargs):
        if 'logged_in' in session:
            ...
            flash("Oncelikle giris yapmalisiniz!")
            return redirect(url_for('login'))
    return wrap

# admin erisim duzeyi icin fonksiyon tanimliyoruz
# kosulun saglanmamasi durumunda uyari veriyor ve
# anasayfa fonksiyonuna yonlendiriyoruz
...
def admin_required(f):
    ...
    def wrap(*args, **kwargs):
        if session.get('admin'):
            ...
            flash("Bu sayfa icin yetkiniz bulunmamaktadir.")
            return redirect(url_for('home_page'))
    return wrap


# veritabaninin yolunu gosteriyoruz
db_uri = 'sqlite:///otel_rezervasyon.sqlite'
engine = create_engine(db_uri)


# uygulamamamiz icin on ayarlari yapilandiriyor ve
# mail onayi icin gerekli ayarlari giriyoruz
class ConfigClass(object):
    ...
    MAIL_USE_SSL = True
    MAIL_USE_TLS = False
    MAIL_USERNAME = 'mail.adresim@gmail.com'  # gmail adresinizi girin
    MAIL_PASSWORD = '********'  # gmail sifrenizi girin
    MAIL_DEFAULT_SENDER = '"Test" <mail.adresim@gmail.com>'
   ...
   # Daha detayli bilgi https://flask-user.readthedocs.io/en/latest/configuring_settings.html de bulunabilir.


# uygulamamizin main fonksiyonunu olusturuyoruz
def create_app():
    """ Flask application factory """

    # ayar yapilandirmamizi ice aktariyoruz
    app = Flask(__name__)
    app.config.from_object(__name__+'.ConfigClass')

    db = SQLAlchemy(app)

    # tablolarimizi olusturuyoruz
    ...
    class Oda(db.Model):
        ...
        otel_id = db.Column(db.Integer, db.ForeignKey(
            'oteller.otel_id', ondelete='CASCADE'))
    ...
    class Sepet(db.Model):
        ...
        user_id = db.Column(db.Integer(), db.ForeignKey(
            'users.kullanici_id', ondelete='CASCADE'))
        oda_id = db.Column(db.Integer(), db.ForeignKey(
            'odalar.oda_id', ondelete='CASCADE'))
        otel_id = db.Column(db.Integer, db.ForeignKey(
            'oteller.otel_id', ondelete='CASCADE'))
    ...
    # veritabanimizi olusturuyor ve tablolara erisim
    # icin takma isimler veriyoruz
    db.create_all()
    ...
    # sistemde admin kayitli degilse bir admin olusturuyoruz
    if not User.query.filter(User.email == 'admin@example.com').first():
        # sifremizi hashli bir sekilde tutmak icin fonksiyona tabi tutuyoruz
        tmp_psw = '12345678'
    ...
    # anasayfaya yonlendirme fonksiyonumuzu olusturuyoruz
    @app.route('/')
    def home_page():
        return render_template("index.html")

    # giris yap fonksiyonumuzu olusturuyoruz,
    # formdan gelen verileri veritabanindaki
    # veriler ile karsilastiriyoruz, uyusmasi
    # durumunda sisteme giris yapiliyor.
    @app.route('/login', methods=['GET', 'POST'])
    def login():
        if request.method == 'POST':
            email = request.form['email']
            # sifremizi veritabaninda ki hashli hali ile karsilastiriyoruz
            ...
            try:
                # boyle bir email ve sifre uyusmasi var mi sorgusu yapiyoruz
                sorgu1 = User.query.filter_by(
                    email=email, password=password).first()
                if sorgu1 is not None:

                    # diger fonksiyonlarda kullanmak uzere girilen emailin kullanici id sini aliyoruz
                    global sorgu_kullanici_id
                    conn = engine.connect()
                    select_st = select([table_users.c.kullanici_id]).where(
                        table_users.c.email == email)
                    sorgu_kullanici_id = conn.execute(select_st).scalar()

                    # bu kullanici admin mi sorgusu yapiyoruz
                    sorgu2 = User.query.filter_by(
                        email=email, password=password, kullanici_rol=1).first()
                ...
                flash('Beklenmeyen bir hata olustu!')
                return redirect(url_for('home_page'))

        else:
            return redirect(url_for('home_page'))

    # login fonksiyonun aynisi fakat yonlendirdigi kisim farkli
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                if sorgu1 is not None:
                    ...
                    # tekrar rezerve fonksiyonuna gitmesini sagliyoruz
    ...
    # cikis yap fonksiyonumuz kullanicinin sepetinin bosaltilmasini sagliyor
    # ve sistemden kullaniciyi cikariyor
    @app.route("/logout")
    @login_required
    def logout():
        conn = engine.connect()
        select_st = table_sepet.delete().where(and_(table_sepet.c.user_id ==
                                                    sorgu_kullanici_id, table_sepet.c.sepet_durum == 0))
        conn.execute(select_st)
        session.clear()
        flash("Basari ile cikis yaptiniz.")

        return redirect(url_for('home_page'))

    # formdan gelen verileri kullanarak veritabanina yeni
    # bir kullanici ekliyoruz
    @app.route('/', methods=['GET', 'POST'])
    def uye_ol():
        if request.method == 'POST':
            # sifreyi hashli bir sekilde tutmayi sagliyoruz
            ...
            try:
                ...
                db.session.commit()
                flash("Kayit basari ile eklendi")
            except:
                flash("Kayit islemi sirasinda hata olustu")

            finally:
                return redirect(url_for('home_page'))

    # adminin sistemde kayitli uyeleri gorebilmesini saglayan fonksiyon yaziyoruz
    @app.route('/uye_gor', methods=['GET', 'POST'])
    @login_required
    @admin_required
    def uye_gor():
        conn = engine.connect()
        # veritabanindan sadece normal uyeleri ceken sorgumuzu yaziyoruz
    ...
    # tablodan gelen kullanici id si ile eslesen kullanici
    # veritabanindan silen fonksiyonu yaziyoruz
    ...
    # veritabaninda varolan otelleri goruntulemeyi ve yeni otel eklemeyi sagliyoruz
    ...
    # tablodan secilen otelin duzenlemesini yapmak icin formdan gelen verilerle
    # otelin bilgileri update sorgusu ile guncelliyoruz
    ...
    def otel_duzenle(id=0):
        if request.method == 'POST':
            ...
            otel_yildizi = request.form['otelyildizi']
            conn = engine.connect()
            select_st1 = table_otel.update().where(table_otel.c.otel_id == id).values(
                otel_adi=otel_adi, otel_sehir=otel_sehir, otel_yildizi=otel_yildizi)
            conn.execute(select_st1)
            return redirect(url_for('otel_listele'))
        else:
            conn = engine.connect()
            select_st2 = select([table_otel.c.otel_adi, table_otel.c.otel_sehir,
                                 table_otel.c.otel_yildizi]).where(table_otel.c.otel_id == id)
            rows = conn.execute(select_st2)
            return render_template("otel_duzenle.html", tmp_otel_id=id, rows=rows)

    # tablodan gelen otel id si ile eslesen oteli
    # veritabanindan siliyoruz
    ...
    # tablodan secilen otel id ile eslesen otele oda ekliyoruz
    @app.route('/oda_listele', methods=['GET', 'POST'])
    @login_required
    @admin_required
    def oda_listele():
        if request.method == 'POST':
            # oda eklemeyi sagliyoruz
            if request.form['btn'] == 'Ekle':
                try:
                    ...
                    flash("Kayit islemi sirasinda hata olustu")
            # otel secimini sagliyoruz
            elif request.form['btn'] == 'Sec':
                ...
                select_st1 = select([table_oda.c.oda_id, table_oda.c.oda_adi, table_oda.c.otel_id,
                                     table_oda.c.oda_tipi, table_oda.c.oda_fiyat]).where(table_oda.c.otel_id == otel_id)
    ...
    # tabloda secilen oda id ile eslesen
    # oda bilgilerini formdan gelen verilerle guncelliyoruz
    ...
    def oda_duzenle(id=0):
        if request.method == 'POST':
            ...
            oda_fiyat = request.form['odafiyat']
            conn = engine.connect()
            select_st1 = table_oda.update().where(table_oda.c.oda_id == id).values(
                oda_adi=oda_adi, oda_tipi=oda_tipi, oda_fiyat=oda_fiyat)
            conn.execute(select_st1)
            return redirect(url_for('oda_listele'))
        else:
            conn = engine.connect()
            select_st2 = select([table_oda.c.oda_id, table_oda.c.oda_adi, table_oda.c.oda_tipi,
                                 table_oda.c.oda_fiyat]).where(table_oda.c.oda_id == id)
            rows = conn.execute(select_st2)

            return render_template("oda_duzenle.html", tmp_oda_id=id, rows=rows)

    # tablodan gelen oda id si ile eslesen odayi
    # veritabanindan siliyoruz
    ...
    # oda durumu 0 olan yani musait olan odalari
    # rezervasyon yapabilmek icin tabloda goruntuluyoruz
    @app.route('/rezerve', methods=['GET', 'POST'])
    def rezerve():
        conn = engine.connect()
        j = table_oda.join(table_otel, table_oda.c.otel_id ==
                           table_otel.c.otel_id)
    ...
    # rezervasyon yapmak icin sectigimiz odayi
    # sepete eklememizi sagliyor
    @app.route('/sepet_ekle/<id>', methods=['GET', 'POST'])
    @login_required
    def sepet_ekle(id=0):
        conn = engine.connect()

        select_st1 = select([table_oda.c.oda_fiyat]).where(
            table_oda.c.oda_id == id)
        toplam_tutar = conn.execute(select_st1).scalar()
        print('toplam tutar: ', toplam_tutar)

        select_st2 = select([table_oda.c.otel_id]).where(
            table_oda.c.oda_id == id)
    ...
    # kullanici puanini ve rezervasyon yaptigmiz oda bilgilerini goruntulememizi sagliyor
    @app.route('/sepet', methods=['GET', 'POST'])
    @login_required
    def sepet():
        global sorgu_kullanici_puan
        conn = engine.connect()
        j = table_oda.join(table_otel, table_oda.c.otel_id == table_otel.c.otel_id).join(
            table_sepet, table_oda.c.oda_id == table_sepet.c.oda_id)
        ...
        sorgu2 = select([table_users.c.kullanici_puan]).where(
            table_users.c.kullanici_id == sorgu_kullanici_id)
    ...
    # tablodan secilen otelin bilgilerini kullanmak icin araci bir fonksiyon yaziyoruz
    # ve sepet_onay_html sayfasina yonlendiriyoz
    ...
    # sepetteki rezervasyon bilgilerini formdan aliyoruz ve guncelliyoruz
    # kullanicinin alisveris durumunda kazanacagi puani hesapliyoruz
    # puan kullanmasi durumunda indirim uygulamayi sagliyoruz
    ...
    def sepet_duzenle(id=None, odaid=None):
        if request.method == 'POST':
            ...
            giris_tarih = date(int(giris_liste[0]), int(
                giris_liste[1]), int(giris_liste[2]))
            cikis_tarih = date(int(cikis_liste[0]), int(
                cikis_liste[1]), int(cikis_liste[2]))
            kalinacak_gun = request.form['kalinacakgun']
            puan_kullanma_durumu = request.form['puan']

            if puan_kullanma_durumu == '1':
                conn = engine.connect()
                select_st2 = select([table_users.c.kullanici_puan]).where(
                    table_users.c.kullanici_id == sorgu_kullanici_id)
            ...
            conn = engine.connect()
            select_st2 = select([table_oda.c.oda_fiyat]).where(
                table_oda.c.oda_id == odaid)
            birim_fiyat = conn.execute(select_st2).scalar()
            print('birim fiyat: ', birim_fiyat)
            toplam_tutar = float(kalinacak_gun) * \
                birim_fiyat - float(puan_kullanma_durumu)
    ...
    # sepette ki rezervasyonumuzu onayliyoruz
    # onaylanan rezervasyon sepetten siliniyor ve
    # islemlerim kismina dusuyor
    ...
    def sepet_onay(id=0):
        ...
        try:
            ...
            select_st1 = table_sepet.update().where(table_sepet.c.sepet_id == id).values(
                sepet_durum=1, rezerve_tarih=tmp_tarihzaman)

            sorgu_kullanici_puan += sorgu_kazanilan_puan
            select_st2 = table_users.update().where(table_users.c.kullanici_id ==
                                                    sorgu_kullanici_id).values(kullanici_puan=sorgu_kullanici_puan)

            conn.execute(select_st1)
            conn.execute(select_st2)
        except:
            flash('Once tarih secmelisiniz')
    ...
    # kullanici id ile eslesen gecmis islemleri goruntuluyoruz
    @app.route('/islemlerim', methods=['GET', 'POST'])
    @login_required
    def islemler():
        conn = engine.connect()
        j = table_sepet.join(table_oda, table_sepet.c.oda_id == table_oda.c.oda_id).join(
            table_otel, table_sepet.c.otel_id == table_otel.c.otel_id)
